...
package org.miamplayer.miamplayerremote.backend.library;
...
public class LibraryDatabaseHelper {
     ...
     * add songs from this db to MiamPlayer. So here we delete the wrong library file.
     *
     * @return true if a database file existed and the current MiamPlayer connection has a different
     * ip that the ip from the library. False otherwise
     */
    public boolean removeDatabaseIfFromOtherMiamPlayer() {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(App.getApp());
        String libraryMiamPlayer = prefs.getString(SharedPreferencesKeys.SP_LIBRARY_IP, "");
        String currentMiamPlayer = prefs.getString(SharedPreferencesKeys.SP_KEY_IP, "");

        if (libraryMiamPlayer.equals(currentMiamPlayer)) {
            return false;
        } else {
            // Save the current library ip
            SharedPreferences.Editor edit = prefs.edit();
            edit.putString(SharedPreferencesKeys.SP_LIBRARY_IP, currentMiamPlayer);
            edit.apply();
