...
public class DownloadManager {
    ...
    private SparseArray<MiamPlayerSongDownloader> mActiveDownloads = new SparseArray<>();

    private SparseArray<MiamPlayerSongDownloader> mFinishedDownloads = new SparseArray<>();
    ...
    private PendingIntent buildNotificationIntent() {
        ...
        intent.putExtra(MiamPlayerMediaSessionNotification.EXTRA_NOTIFICATION_ID,
    ...
    public boolean addJob(MiamPlayerMessage miamPlayerMessage) {
        MiamPlayerSongDownloader songDownloader = new MiamPlayerSongDownloader();
        ...
        songDownloader.startDownload(miamPlayerMessage);

        return true;
    }

    public List<MiamPlayerSongDownloader> getAllDownloaders() {
        LinkedList<MiamPlayerSongDownloader> downloaders = new LinkedList<>();
    ...
    public String getTitleForItem(MiamPlayerSongDownloader downloader) {
    ...
    public String getSubtitleForItem(MiamPlayerSongDownloader downloader) {
        DownloadStatus status = downloader.getDownloadStatus();
    ...
    private void setNotificationSingleDownload(NotificationCompat.Builder notificationBuilder,
        ...
        MiamPlayerSongDownloader downloader = mActiveDownloads.get(status.getId());
    ...
    private void setNotificationMultipleDownloads(NotificationCompat.Builder notificationBuilder) {
        ...
        int progress = 0;
        for (int i = 0; i < mActiveDownloads.size(); ++i) {
            MiamPlayerSongDownloader downloader = mActiveDownloads.valueAt(i);
            DownloadStatus status = downloader.getDownloadStatus();
